...
(This is not a true history and is only a parable. Also it is definitely not my own invention but I can't figure out who the original source for this story is.)
...
2. If all the sheep you see are in the enclosure and there are still stones in the bag, you're missing some sheep. One sheep per stone remaining is missing.
...
1. If the sequence *starts* with \(-\) then something has gone wrong - we've tried to take a rock out of an empty bad, or a sheep out of a world with no sheep in it.
...
Then any number other than \(0\) can be written as \(S(S(\ldotsS(0)\ldots))\) by rule \(4\),
...
The careful reader will notice a slight of hand there:
