...
Our inductive property is that \(\sigma\limits_{i = 1}^n i = \frac{n(n + 1)}{2}\). This is true for \(1\), you can just check that by calculating \(\frac{2}{2}\).

Suppose it's true for \(n - 1\). That is, \(\sigma\limits_{i = 1}^{n - 1} i = \frac{(n - 1)n}{2}\). Then \(\sigma\limits_{i = 1}^n i = n + \sigma\limits_{i = 1}^{n - 1} i = n + \frac{n(n -1)}{2} = \frac{n(n + 1){2}\).^[This is in fact the same calculation we did in the even/odd case above.] So the property holds for \(n\) as well, and the result is proved by induction.
